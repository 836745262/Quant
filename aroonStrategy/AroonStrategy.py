"""
这里的Demo是一个最简单的Aroon策略实现
"""
from vnpy.trader.vtConstant import *
from vnpy.trader.app.ctaStrategy import CtaTemplate
import talib as ta
from datetime import datetime
from AroonSignal import AroonSignal

########################################################################
# 策略继承CtaTemplate
class AroonStrategy(CtaTemplate):
    """Aroon策略Demo"""
    className = 'AroonStrategy'
    author = 'BrianLee'
    ...
    def exitSignal(self, signalPeriod):
        ...
        CrossSignal = 0
        if arrayPrepared1:
            algorithm = AroonSignal()
            CrossSignal, aroondown, aroonup, aroon = algorithm.AroonCross(amSignal, self.paraDict)
        return CrossSignal
    ...
    def entrySignal(self,signalPeriod):
        ...
        if arrayPrepared1:
            ...
            self.chartLog['aroonup'].append(aroonup[-1])
            self.chartLog['aroon'].append(aroon[-1])
            self.chartLog['aroondown'].append(aroondown[-1])
        return entrySignal
