"""
这里的Demo是一个最简单的Kdj策略实现
"""
from vnpy.trader.vtConstant import *
from vnpy.trader.app.ctaStrategy import CtaTemplate
import talib as ta
from datetime import datetime
from KdjSignal import KdjSignal
...
class KdjStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        # 首先找到策略的父类（就是类CtaTemplate），然后把KdjStrategy的对象转换为类CtaTemplate的对象
    ...
    def exitSignal(self, signalPeriod):
        ...
        if arrayPrepared1:
            algorithm = KdjSignal()
            CrossSignal, sig_k, sig_d, sig_j = algorithm.kdjCross(amSignal, self.paraDict)
        return CrossSignal
    ...
    def entrySignal(self,signalPeriod):
        ...
        if arrayPrepared1:
            ...
            self.chartLog['sig_k'].append(sig_k[-1])
            self.chartLog['sig_d'].append(sig_d[-1])
        return entrySignal
