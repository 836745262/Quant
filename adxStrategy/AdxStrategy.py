"""
这里的Demo是一个最简单的Adx策略实现
"""
from vnpy.trader.vtConstant import *
from vnpy.trader.app.ctaStrategy import CtaTemplate
import talib as ta
from datetime import datetime
from AdxSignal import adxSignal

########################################################################
# 策略继承CtaTemplate
class AdxStrategy(CtaTemplate):
    """Adx策略Demo"""
    className = 'AdxStrategy'
    author = 'BrianLee'
    ...
    def __init__(self, ctaEngine, setting):
        # 首先找到策略的父类（就是类CtaTemplate），然后把AdxStrategy的对象转换为类CtaTemplate的对象
    ...
    def exitSignal(self, signalPeriod):
        ...
        if arrayPrepared1:
            algorithm = adxSignal()
            CrossSignal, PLUS_DI, ADX, MINUS_DI = algorithm.adxsignal(amSignal, self.paraDict)
        return CrossSignal
    ...
    def entrySignal(self,signalPeriod):
        ...
        if arrayPrepared1:
            ...
            self.chartLog['PLUS_DI'].append(PLUS_DI[-1])
            self.chartLog['ADX'].append(ADX[-1])
            self.chartLog['MINUS_DI'].append(MINUS_DI[-1])
        return entrySignal
